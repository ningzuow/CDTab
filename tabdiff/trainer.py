...
# TabDiff训练逻辑的核心控制器，把所有组件串联成一个完整的训练管线
...
class Trainer:
    ...
    def __init__(
        ...
        # 建立EMA副本，每步更新一次指数滑动平均参数。让模型在训练后期更稳定、生成质量好
        self.y_only = y_only
        ...
        #设置优化器，这个很重要，它包括了denoise_fn和两个noise_schedule的参数
    ...
    def _run_step(self, x, closs_weight, dloss_weight):
        ...
        self.diffusion.train() # 切换到训练模式

        self.optimizer.zero_grad() # 清空梯度

        dloss, closs = self.diffusion.mixed_loss(x)# 计算损失
        # 也就是加噪+返回损失
