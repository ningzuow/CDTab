...
def main(args):
    ...
    ## Load Metrics
    # 'density'是专为生成模式设计的一个分布相似性指标，训练阶段使用
    ...
    # y_only 是「只学目标变量」的扩散模型，配合主模型更好的生成目标变量与特征一致的样本
    # 目前而言，仅在 插补（impute）路径需要：将专门学 y 的模型作为条件指导（CFG 风格）来修补缺失值。
    # 疑问一：为什么我反复为GPT，它都说CFG不可以直接进行大量的少数类生成？
    if args.y_only:
    ...
    # 模式分支：
    # test + report：重复采样多次，输出均值/方差报告等等......
    if args.mode == 'test':
        if args.report:
            if  is_dcr:
