...
class Tokenizer(nn.Module):
    # 参数表
    # d_numerical: 数值特征的个数
    # categories: 类别特征的类别数列表[K1, K2, ..., Kn],每个值表示对应类别特征的取值个数
    # d_token: token的维度
    # bias: 是否使用偏置项
    def __init__(self, d_numerical, categories, d_token, bias):
        ...
        # self.weight 的形状为 (d_numerical + 1, d_token)，对应数值特征的权重矩阵
        # self.bias 的形状为 (d_bias, d_token)，对应偏置项矩阵
        # CLS 是人为加的第一个 token，代表整行样本的综合信息，方便 Transformer 建模整体结构。
    ...
    def forward(self, x_num, x_cat):
        ...
        # 生成数值token
        # x 的形状为 (batch_size, n_tokens, d_token)
        # 每个数值列都有一个专属方向向量 w_i
        # Token 向量 = 数值 * w_i
        ...
        # 生成类别token
        # 直观理解：
        # 类别列的 one-hot × 类别 embedding 矩阵
        # → 选出该取值的嵌入向量
        #→ 就是这个类别列的 token。
        # 其实质上是一个查表，one-hot 向量中只有一个位置为 1，其余为 0
...
class MultiheadAttention(nn.Module):
    def __init__(self, d, n_heads, dropout, initialization = 'kaiming'):
        ...
        super().__init__()
        #三个可训练的线性变换，把每个token的表示分别映射到三个不同的空间
        # Q（查询）、K（键）、V（值）
...
# Transfomer的具体实现，包括多层注意力机制和前馈神经网络
# 与经典的Transformer类似，但有一些调整以适应表格数据的特点
# 最终输出通过一个线性层映射到所需的输出维度        
...
# 重构器，将Transformer的输出重新映射回原始数据空间
# 返回重构后的数值特征和类别特征预测
