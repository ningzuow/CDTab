...
class UnifiedCtimeDiffusion(torch.nn.Module):
    # 初始化，设置噪声scheduler、记录数值、类别特征信息等
    def __init__(
            self,
        ...
        # 创建两个噪声调度器对象，如果选择_per_column，则为每个类别/数值特征学习单独的噪声调度参数
    ...
    # 对x加噪（离散和连续）；调用模型预测x0；计算损失（离散和连续）--------训练时用
    def mixed_loss(self, x):
        ...
        # Sample noise level
        # 加噪阶段，采样一个随机时间步，并据此计算噪声强度σ，不同特征有不同的噪声曲线，具体取决于num_schedule和cat_schedule
        ...
        # 但有时候时间步均匀，噪声未必均匀，所以这里提供了一个对数正态采样噪声，可以学习更多的高噪声样本
        # 二者并不冲突    
        else:
        ...
        # 计算掩码概率（离散特征）
        # 对离散变量，用 move_chance 表示“这个特征在本次扩散中被替换成 [MASK] 的概率”
        ...
        # 离散特征随机替换为 mask
        # 如果调度器是可学习的（_per_column），用 soft mask（Gumbel-softmax 平滑替换）
        # 否则用 hard mask（直接替换为 mask 索引）
        # 两者的区别在于，soft mask 在反向传播时可以传递梯度，hard mask 则不行
        if x_cat.shape[1] > 0:
        ...
        # 连续特征损失，用 EDM loss，在后面有这个辅助函数，本质是加权MSE
        # 离散特征损失，用 absorbed cross-entropy loss，在后面有这个辅助函数
        if x_num.shape[1] > 0:
    ...
    # 初始化随机噪声，调用 edm_update() 反向去噪-------采样生成
    # 从先验噪声出发，沿着扩散反向过程一步步去噪，最终得到新表格样本
    @torch.no_grad()
    def sample(self, num_samples):
        ...
        # 随机扰动采样---->基于反向随机采样器的采样
        # 在每步反向去噪前，先把 σ 轻微抬高（“churn”），再回落，有助于多样性与稳定性（Karras EDM 的 trick）
        # 通过构造三元组（σ_cur, σ_next, σ_hat）实现
        ...
        逐步反向：
            ...
            传入当前步相关的时间与噪声三元组（连续/离散各一套）；

            交给 edm_update() 完成本步的去噪更新：

                前向小扰动(若启用 churn):通过上面的判断，如果未启用churn，sigma_num_hat = sigma_num_cur

                喂入 denoise 网络得到去噪预测

                (可选)y_only CFG 引导

                Euler 连续更新（必要时二阶修正在 edm_update 内处理）

                离散通过 _mdlm_update() 基于 logits 进行采样更新（mask → 真值）

        q_xs 是离散分支中间量（每步的过渡概率），这里主要用于诊断或可视化。
        '''
    ...
    # 单步去噪；根据当前噪声水平预测 denoised 结果，应用 y_only 引导 (CFG)，Euler 更新连续特征，更新离散特征 (mask sampling)
    def edm_update(
