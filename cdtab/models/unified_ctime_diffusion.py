...
import math
import numpy as np
from cdtab.models.noise_schedule import *
...
class UnifiedCtimeDiffusion(torch.nn.Module):
    ...
    def mixed_loss(self, x_num, x_cat, y):
        ...
        if self.noise_dist == "uniform_t":
            ...
            sigma_num = self.sample_ctime_noise(x_num)      
            t = self.num_schedule.inverse_to_t(sigma_num)
            while torch.any((t < 0) + (t > 1)):
                invalid_idx = ((t < 0) + (t > 1)).nonzero().squeeze(-1)
                sigma_num[invalid_idx] = self.sample_ctime_noise(x_num[:len(invalid_idx)])
                t = self.num_schedule.inverse_to_t(sigma_num)
        ...
        move_chance = -torch.expm1(-sigma_cat)      # 1 - exp(-sigma)

        x_num_t = x_num
        if x_num.shape[1] > 0:
            noise = torch.randn_like(x_num)
            if sigma_num.dim() == 1:
                sigma_num_exp = sigma_num.view(b, 1)
            else:
                sigma_num_exp = sigma_num
            x_num_t = x_num + noise * sigma_num_exp
        ...
        if sigma_num.dim() == 1:
            sigma_for_model = sigma_num.view(b, 1)
        else:
            sigma_for_model = sigma_num
        ...
        d_loss = torch.zeros((1,), dtype=torch.float32, device=device)
        c_loss = torch.zeros((1,), dtype=torch.float32, device=device)

        if x_num.shape[1] > 0:
            c_loss = self._edm_loss(model_out_num, x_num, sigma_for_model)
    ...
    def edm_update(
        ...
        denoised, raw_logits = self._denoise_fn(
            ...
            cond_label = torch.full((b,), self.sample_label, dtype=torch.long, device=x_num_hat.device)# ← 采样用的固定标签（见改动 4）
        )
