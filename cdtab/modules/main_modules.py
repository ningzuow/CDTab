...
from cdtab.modules.transformer import Reconstructor, Tokenizer, Transformer
import torch
...
class MLPDiffusion(nn.Module):
    ...
    def forward(self, x, timesteps, y_embed=None):
...
class UniModMLP(nn.Module):
    ...
    def forward(self, x_num, x_cat, timesteps, cond_label=None):
        # 1. Tokenize
        e = self.tokenizer(x_num, x_cat)   # [B, 1 + tokens, d_token]


        decoder_input = e[:, 1:, :]        
        h = self.encoder(decoder_input)

        pred_h = self.mlp(h.reshape(h.size(0), -1), timesteps, None)

        pred_e = self.decoder(pred_h.reshape(*h.shape))
        x_num_pred, x_cat_pred = self.detokenizer(pred_e)
        x_cat_pred = torch.cat(x_cat_pred, dim=-1) if len(x_cat_pred)>0 else \
                    torch.zeros_like(x_cat).to(x_num_pred.dtype)
...
class Precond(nn.Module):
    ...
    def forward(self, x_num, x_cat, t, sigma, cond_label=None):
        ...
        if self.net_conditioning == "sigma":
            F_x, x_cat_pred = self.denoise_fn_F(
                x_in, x_cat, c_noise.flatten()
            )
...
class Model(nn.Module):
    ...
    def forward(self, x_num, x_cat, t, sigma=None, cond_label=None):
        if self.precond:
            return self.denoise_fn_D(x_num, x_cat, t, sigma, cond_label=cond_label)
        else:
            return self.denoise_fn_D(x_num, x_cat, t, cond_label=cond_label)
