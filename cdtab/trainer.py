...
from cdtab.modules.contrastive_modules import ProjectionMLP, PredictionMLP
import torch.nn.functional as F
from cdtab.modules.contrastive_loss import compute_contrastive_loss
...
class Trainer:
    def __init__(
        ...
        if contrast_modules is not None:
            self.tokenizer_q = contrast_modules['tokenizer_q']
            self.encoder_q   = contrast_modules['encoder_q']
            self.proj_q      = contrast_modules['proj_q']
            self.pred_q      = contrast_modules['pred_q']

            self.tokenizer_k = contrast_modules['tokenizer_k']
            self.encoder_k   = contrast_modules['encoder_k']
            self.proj_k      = contrast_modules['proj_k']

        params = list(self.diffusion.parameters())
        if self.contrast_modules is not None:
            params += list(self.tokenizer_q.parameters())
            params += list(self.encoder_q.parameters())
            params += list(self.proj_q.parameters())
            params += list(self.pred_q.parameters())
        ...
        if dataset is not None and hasattr(dataset, "categories"):
            self.categories_raw = np.array(dataset.categories, dtype=int)
        else:
            self.categories_raw = None
        with open(f"data/{dataset.dataname}/pair_info.json", "r") as f:
    ...
    def _run_step(self, batch, closs_weight, dloss_weight):
       

        # batch: {"x": tensor(B,D), "y": tensor(B), "index": tensor(B)}
        x_full = batch["x"].float().to(self.device)
        y_full = batch["y"].long().to(self.device)
        batch_indices = batch["index"]

        B = x_full.size(0)
        half_B = B // 2

        x_anchor = x_full[:half_B]                     # [half_B, D]
        y_anchor = y_full[:half_B]                     # [half_B]
        anchor_indices = batch_indices[:half_B]        # [half_B]

        partner_indices_list = []
        for idx_global in anchor_indices:
            idx_global = int(idx_global)
            pid = int(self.pair_info[str(idx_global)])   
            partner_indices_list.append(pid)

        partner_indices = torch.tensor(partner_indices_list, dtype=torch.long, device=self.dataset.X.device)
        x_partner = self.dataset.X[partner_indices]
        x_partner = x_partner.to(self.device)

        y_partner = self.dataset.y[partner_indices].to(self.device)# [half_B]

        x = torch.cat([x_anchor, x_partner], dim=0)     # [B, D]
        labels = torch.cat([y_anchor, y_partner], dim=0)  # [B]

        pairs = [(i, i + half_B) for i in range(half_B)]


        num_dim = self.diffusion.num_numerical_features
        x_num = x[:, :num_dim].float()
        x_cat_idx = x[:, num_dim:].long()


        num_classes_list = list(self.categories_raw)   
        one_hot_list = []
        for j, Kj in enumerate(num_classes_list):
            col_idx = x_cat_idx[:, j]                         
            one_hot = F.one_hot(col_idx, num_classes=Kj)      
            one_hot_list.append(one_hot.float())
        x_cat_oh = torch.cat(one_hot_list, dim=1)             


        self.optimizer.zero_grad()
        contrastive_loss = torch.zeros((), device=x.device)

        if self.contrast_modules is not None:
            contrastive_loss, z_q, p_q, z_k = compute_contrastive_loss(
                x_num, x_cat_oh, pairs, labels,
                self.tokenizer_q, self.encoder_q, self.proj_q, self.pred_q,
                self.tokenizer_k, self.encoder_k, self.proj_k
            )

        contrastive_loss = contrastive_loss * 500
        minority_mask = (labels == 1)
        if minority_mask.sum() > 0:
            dloss, closs = self.diffusion.mixed_loss(
                x_num[minority_mask],
                x_cat_idx[minority_mask],
                labels[minority_mask]
            )
        else:
            dloss = torch.zeros((), device=x.device)
            closs = torch.zeros((), device=x.device)


        loss = dloss_weight * dloss + closs_weight * closs + contrastive_loss
        loss.backward()
        self.optimizer.step()

        if self.contrast_modules is not None:
            m = 0.999
            for p_k, p_q in zip(self.tokenizer_k.parameters(), self.tokenizer_q.parameters()):
        ...
        return dloss, closs, contrastive_loss
        ...
        with torch.no_grad():
            for batch in data_iter:
                ...
                if minority_mask.sum() > 0:
                    ...
                    Bm = minority_mask.sum().item()
                    curr_dloss += batch_dloss.item() * Bm
                    curr_closs += batch_closs.item() * Bm
                    curr_count += Bm
    ...
    def run_loop(self):
        ...
        for epoch in range (start_epoch, self.steps):
            ...
            for batch in pbar:
                batch_dloss, batch_closs, contrastive_loss = self._run_step(
                    batch,
                    closs_weight,
                    dloss_weight
                )
                ...
                curr_dloss += batch_dloss.item() * B
                curr_closs += batch_closs.item() * B
                curr_count += B
                ...
                pbar.set_postfix({
                    ...
                    "Contrast": float(contrastive_loss.detach().cpu()), # 看一下对比损失
            ...
            if total_loss < best_loss and self.curr_epoch > 1000:
                ...
                if self.contrast_modules is not None:
                    ...
                    contrastive_q_state = {
                        "tokenizer": self.tokenizer_q.state_dict(),
                        "encoder": self.encoder_q.state_dict(),
                        "proj": self.proj_q.state_dict(),
                        "pred": self.pred_q.state_dict()
                    }
                    ...
                    torch.save(
                        ...
                        os.path.join(
                            ...
                            f"contrastive_q_full_{epoch+1}.pt"
                        )
                    )
                
                    contrastive_k_state = {
                        "tokenizer": self.tokenizer_k.state_dict(),
                        "encoder": self.encoder_k.state_dict(),
                        "proj": self.proj_k.state_dict()
                    }
                    ...
                    torch.save(
                        ...
                        os.path.join(
                            self.model_save_path,
                            f"contrastive_k_full_{epoch+1}.pt"
                        )
                    )
            ...
            if ema_total_loss < best_ema_loss and self.curr_epoch > 1000:
...
def split_num_cat_target(syn_data, info, num_inverse, int_inverse, cat_inverse):
    ...
    # 训练时你已经摘掉 label，所以扩散只生成 num+cat，总长度 = num + cat
    n_num_feat = len(num_col_idx)
    n_cat_feat = len(cat_col_idx)

    syn_num = syn_data[:, :n_num_feat]
    syn_cat = syn_data[:, n_num_feat:n_num_feat + n_cat_feat]
...
def recover_data(syn_num, syn_cat, syn_target, info):
    ...
    total_no_target = len(num_col_idx) + len(cat_col_idx)

    for i in range(total_no_target):
