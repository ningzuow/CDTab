...
import numpy as np
from cdtab.metrics import TabMetrics
from cdtab.modules.main_modules import UniModMLP
from cdtab.modules.main_modules import Model
from cdtab.models.unified_ctime_diffusion import UnifiedCtimeDiffusion
from cdtab.trainer import Trainer
import src
import torch
...
def main(args):
    ...
    config_path = f'{curr_dir}/configs/cdtab_configs.toml'
    ...
    if args.y_only:
        ...
        if main_model_configs['diffusion_params']['scheduler'] == "power_mean_per_column": # if learnable schedule is enabled in the main model, we need to infer noise params of the target column from the main model ckpt and train the y_only model with those params
            from cdtab.models.noise_schedule import PowerMeanNoise_PerColumn, LogLinearNoise_PerColumn
    ...
    d_token = raw_config['unimodmlp_params']['d_token']
    d_num   = raw_config['unimodmlp_params']['d_numerical']
    d_cat   = len(raw_config['unimodmlp_params']['categories'])
    encoder_out_dim = d_token * (d_num + d_cat)
    
    tokenizer_q = backbone.tokenizer
    encoder_q   = backbone.encoder

    proj_q = ProjectionMLP(in_dim=encoder_out_dim)
    pred_q = PredictionMLP(in_dim=256)


    tokenizer_k = deepcopy(tokenizer_q)
    encoder_k   = deepcopy(encoder_q)
    proj_k      = deepcopy(proj_q)

    for p in tokenizer_k.parameters(): p.requires_grad = False
    for p in encoder_k.parameters():   p.requires_grad = False
    for p in proj_k.parameters():      p.requires_grad = False

    contrast_modules = dict(
        tokenizer_q = tokenizer_q,
        encoder_q   = encoder_q,
        proj_q      = proj_q,
        pred_q      = pred_q,

        tokenizer_k = tokenizer_k,
        encoder_k   = encoder_k,
        proj_k      = proj_k,
    )

    for name, module in contrast_modules.items():
        module.to(device)
    ...
    ## Enable Wandb
    project_name = f"cdtab_{dataname}"
    ...
    trainer = Trainer(
        ...
        **raw_config['train']['main'],
        contrast_modules=contrast_modules,
    ...
    if args.mode == 'test':
        ...
        num_samples = (
            ...
            if args.num_samples_to_generate is not None
            else train_data.X.shape[0]
        )
    
        num_runs = 10   
    
        print(f"[Sampling Only] Generating {num_samples} samples, repeated {num_runs} runs...")
    
        save_dir = result_save_path
        os.makedirs(save_dir, exist_ok=True)
    
        for run_id in range(num_runs):
            print(f"\n[Sampling Only] Run {run_id}/{num_runs - 1}")
            ...
            save_path = os.path.join(save_dir, f"samples_{run_id}.csv")
            syn_df.to_csv(save_path, index=False)
    
            print(f"[Sampling Only] Samples saved to {save_path}")
